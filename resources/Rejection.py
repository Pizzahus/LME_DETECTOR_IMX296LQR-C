...
class RejectionWorker(QThread):
    ...
    def __init__(self, sensorPin: int, rejectPin: int, startReject: int=0, inputPullUp: bool=True, rejectDelay: float=0.5, rejectActiveHigh: bool=True):
        ...
        # Sensor
        self.sensor = Button(sensorPin, pull_up=inputPullUp, bounce_time=0.05)
    ...
    def _reject(self, item: Item):
        ...
        self.rejector.on()
        time.sleep(0.2)  # ระยะเวลาเปิด reject
        self.rejector.off()
        self.rejected_signal.emit(item.tag)

    def run(self):
        while self.running:
            time.sleep(0.1)  # idle loop, sensor trigger ทำงานผ่าน callback
