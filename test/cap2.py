...
X1, Y1, X2, Y2 = 230, 180, 420, 290

picam2 = Picamera2()
sensor = Button(23, pull_up=True, bounce_time=0.05)  # ตั้งค่า bounce_time = 0.1 วินาที

config = picam2.create_still_configuration(main={"size": (1024, 768)}, buffer_count=4)
...
def captured():
    ...
    text, processed_img = detect_and_recognize_text(inverted_image)
...
while True:
    ...
    if key == ord("q"):
        ...
        captured()
    elif sensor.is_pressed:  # ตรวจสอบสถานะปุ่มแบบ polling
        captured()
